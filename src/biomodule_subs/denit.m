...
fNH3 = fdecomp(B.alphaNH4, bv(:,:,B.idx.nh4)); %nz x nx
...
src = denidx(i,1);
snk = denidx(i,2);
%if isequal([src snk],[B.idx.no3 B.idx.mys])
%td = tempdep(B.vdec(B.idx.pon,B.idx.nh4), B.Kdec(B.idx.pon,B.idx.nh4), P.T);% nz x nx
%den(src,snk,:,:) = permute(B.RNo3PON.*td.*(1-fo2).*fNo3.*(1-fNH3).*bv(:,:,src), [4 3 1 2]);
%else
td = tempdep(B.vdec(src,snk), B.Kdec(src,snk), P.T);% nz x nx
den(src,snk,:,:) = permute(td.*(1-fo2).*fNo3.*(1-fNH3).*bv(:,:,src), [4 3 1 2]);
%end
end
