...
if ~isstruct(varargin{end}),
    if isequal(varargin{end},'sorted'),
        ...
        error('mergestruct:InvalidArgument','Last argument should be a structure, or the string "sorted".') ;
    end
else
    sorted = 0 ;
end
...
for ii=1:N,
    X = varargin{ii} ;
    if ~isstruct(X),
        error('mergestruct:InvalidArgument',['Argument #' num2str(ii) ' is not a structure.']) ;
    end
    ...
    if ~isempty(X),
        ...
        if ii > 1 && ~isempty(sz0)
            if ~isequal(size(X), sz0)
                error('mergestruct:UnequalSizes','All structures should have the same size.') ;
            end
        else
            sz0 = size(X) ;
        end
...
elseif NonEmptyInputsN == 1,
    ...
    if numel(UFN) ~= numel(FN),
        warning('mergestruct:DuplicatesFound','Fieldnames are not unique between structures.') ;
        sorted = 1 ;
    end
