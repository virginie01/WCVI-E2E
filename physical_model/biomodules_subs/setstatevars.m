function [names, nbsv, nemidx, extrazooidx, nekidx, A] = setstatevars(BioIn)
...
if ~BioIn.isnem
    ...
    [~, nemidx] = ismember(nemnames(:,1), names(:,1));
    [~, extrazooidx]=ismember(names(isz,1),names(:,1));
    [~, nekidx]=ismember(names(isn,1),names(:,1));
    ...
    A.isdet = ismember(names(:,1), {'PON','Opal'});
...
%--------------------------
% Movement of nekton groups
%--------------------------

A.nekdist = BioIn.nekdist;
