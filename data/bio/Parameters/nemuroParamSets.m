...
%sets = Ptable.Properties.VariableNames(3:2:end); % names of each set
unit = Ptable{3:end,1};
%unit = Ptable{1:end,1};

nemparam = Ptable.Properties.RowNames(3:end);
%nemparam = Ptable.Properties.RowNames(1:end);

setdata = table2array(Ptable(3:end,2:2:end));
%setdata = table2array(Ptable(1:end,2:2:end));
setunit = table2array(Ptable(1:end,3:2:end));
